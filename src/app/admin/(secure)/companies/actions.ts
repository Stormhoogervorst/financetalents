...
//
// Verwijdert een company volledig vanuit het admin-dashboard.
//
// Flow:
...
export async function deleteEmployerAction(
  ...
  if (firmError) {
    ...
    return { error: "Company ophalen mislukt." };
  }
  if (!firm) {
    return { error: "Company niet gevonden." };
  }
  ...
  // Guard: weiger het verwijderen van andere admin-accounts. Company-
  // delete is nadrukkelijk voor gewone companies, niet voor collega-
  ...
  if (ownerProfile?.role === "admin") {
    return {
      error:
        "This company is linked to an admin account and cannot be deleted from this screen.",
    };
  }
  ...
  try {
    ...
    if (deleteUserError) {
      ...
      if (!notFound) {
        ...
        return { error: "Company verwijderen is mislukt." };
      }
  ...
  // Force een verse render van de companieslijst + dashboard.
  ...
  redirect("/admin/companies?deleted=" + encodeURIComponent(firm.name));
}
