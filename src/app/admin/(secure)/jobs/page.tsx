...
/**
 * Centraal job-overzicht voor de Super Admin.
 *
 * Haalt álle jobs op (nieuwste eerst) via de service-role client en
...
export default async function AdminJobsPage({
  ...
  if (error) {
    console.error("[admin/jobs] jobs fetch error:", error.message);
  }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div>
        ...
        <h1 className="mt-3 text-2xl font-bold text-gray-900">Jobbeheer</h1>
        <p className="mt-1 text-sm text-gray-500">
          All jobs op het platform. Filter op status of verwijder
          ongewenste posts.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {FILTERS.map(({ key, label }) => {
          ...
          const href =
            key === "all" ? "/admin/jobs" : `/admin/jobs?filter=${key}`;
          return (
            <Link
              key={key}
              href={href}
      ...
      <div className="rounded-2xl bg-white ring-1 ring-gray-200 shadow-sm overflow-hidden">
        ...
        {jobs.length === 0 ? (
          <div className="px-6 py-16 text-center text-sm text-gray-400">
            ...
            <p className="mt-2">No jobs found for this filter.</p>
          </div>
        ) : (
