...
import { buildJobPostingSchema } from "@/lib/schemas/jobPosting";
...
export interface JobsSearchParams {
  ...
  sector?: string;
  role?: string;
  /** Legacy Dutch query params kept readable so old filtered links still work after redirect. */
  locatie?: string;
  straal?: string;
  type?: string;
  rechtsgebied?: string;
...
function buildJobQuery(
  ...
  let query = supabase
    .from("jobs")
    .select("*, firms ( name, logo_url, slug, website_url )")
    .eq("status", "active")
    .in("type", typeValues);
  ...
  } else if ((params.sector ?? params.rechtsgebied)) {
    query = query.ilike("practice_area", `%${(params.sector ?? params.rechtsgebied)}%`);
  }
  ...
  const locationFilter = fixedLocation ?? (params.location ?? params.locatie);
  ...
  if ((params.role ?? params.functie)) {
    query = query.ilike("title", `%${(params.role ?? params.functie)}%`);
  }
...
export default async function JobsListingPage({
  ...
  const radiusKm = parseInt((params.radius ?? params.straal) ?? "0", 10) || 0;
  const useGeo = !!((params.location ?? params.locatie) && radiusKm > 0);
  const geo = useGeo ? await geocodeCity((params.location ?? params.locatie)!) : null;
  ...
  type JobWithFirm = Omit<Job, "firms"> & {
    firms: (JobFirmPreview & { website_url: string | null }) | null;
  };
  ...
  const itemListJsonLd = itemListSchemaName
    ? {
        ...
        itemListElement: jobList.map((job, index) => ({
          "@type": "ListItem",
          position: index + 1,
          item: buildJobPostingSchema(job, job.firms ?? null),
        })),
      }
    : null;
  ...
  const hasFilters = !!(
    params.q ||
    (!fixedLocation && (params.location ?? params.locatie)) ||
    params.type ||
    (!fixedPracticeArea && (params.sector ?? params.rechtsgebied)) ||
    (params.role ?? params.functie) ||
    ((params.radius ?? params.straal) && (params.radius ?? params.straal) !== "0")
  );

  const filterParts: string[] = [];
  if (params.q) filterParts.push(params.q);
  const roleFilter = params.role ?? params.functie;
  const sectorFilter = params.sector ?? params.rechtsgebied;
  const locationFilterLabel = params.location ?? params.locatie;
  if (roleFilter) filterParts.push(roleFilter);
  if (!fixedPracticeArea && sectorFilter) {
    filterParts.push(sectorFilter);
  }
  if (fixedLocation) filterParts.push(fixedLocation);
  else if (locationFilterLabel) filterParts.push(locationFilterLabel);
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-[#EBEBEB] text-[#222222]">
            ...
            <div className="grid grid-cols-1 border border-[#222222] bg-[#EBEBEB] md:grid-cols-2 xl:grid-cols-[minmax(240px,1.3fr)_minmax(180px,0.85fr)_minmax(96px,0.45fr)_minmax(160px,0.7fr)_minmax(190px,0.9fr)_auto]">
              ...
              {fixedLocation ? (
                ...
                >
                  ...
                  <input
                    id="filter-location"
                    name="location"
                    defaultValue={(params.location ?? params.locatie) ?? ""}
              ...
              <div className="flex min-h-[72px] items-center border-b border-[#222222] bg-white px-5 md:border-r xl:border-b-0">
                {fixedLocation ? (
                  ...
                  <RadiusSelect
                    name="radius"
                    defaultValue={(params.radius ?? params.straal) ?? "0"}
              ...
              {fixedPracticeArea ? (
                ...
                >
                  <select
                    id="filter-sector"
                    name="sector"
                    defaultValue={(params.sector ?? params.rechtsgebied) ?? ""}
                    className="w-full appearance-none border-none bg-transparent py-3 pr-7 text-[14px] text-[#222222] outline-none focus:outline-none"
                  >
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          {jobList.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {jobList.map((job) => (
                <JobGridCard key={job.id} job={job} stageMode={cardStageMode} />
              ))}
            </div>
          ) : (
      ...
      {seoIntroContent && (
        ...
        >
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 gap-8 border-t border-[#222222]/20 pt-10 lg:grid-cols-12">
            <div className="lg:col-span-4">
              <p className="ft-display text-[15px] tracking-[-0.02em] text-[#222222]/60">
                Market overview
              </p>
            </div>
            <div className="max-w-[760px] text-[17px] leading-[1.65] text-[#222222]/70 lg:col-span-8">
              {seoIntroContent}
            </div>
          </div>
        </section>
      ...
      {internalLinksSection && internalLinksSection.links.length > 0 && (
        <section
          style={{
            paddingLeft: "clamp(24px, 5vw, 80px)",
            paddingRight: "clamp(24px, 5vw, 80px)",
        ...
        >
          <div className="max-w-[1400px] mx-auto">
            <div className="border border-[#222222] bg-white p-6 md:p-8">
              <h2 className="ft-display text-[clamp(32px,4vw,56px)] font-extrabold leading-[0.95] tracking-[-0.06em] text-[#222222]">
                {internalLinksSection.heading}
              </h2>
              <div className="mt-8 flex flex-wrap gap-3">
                {internalLinksSection.links.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="rounded-full border border-[#222222] px-4 py-2 text-[14px] font-medium text-[#222222] transition-colors duration-200 hover:border-[#E85A00] hover:text-[#E85A00]"
                  >
                    {link.label}
                  </Link>
                ))}
